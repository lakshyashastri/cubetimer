...
const useCubeTimer = () => {
	...
	useEffect(() => {
		const loadedTimes = JSON.parse(
			localStorage.getItem("cubeTimerTimes") || "[]"
		);
		setTimes(loadedTimes);
		const loadedBestAverages = JSON.parse(
			localStorage.getItem("cubeTimerBestAverages") || "{}"
		);
	...
	const calculateAverageFromValues = useCallback((timeValuesInput, N) => {
		...
		const timeValues = timeValuesInput.filter(
		...
		if (N >= 5) {
			...
			return (
				trimmedTimes.reduce((sum, t) => sum + t, 0) /
				trimmedTimes.length
			);
		} else {
			return (
	...
	const calculateCurrentAverage = useCallback(
		N => {
	...
	const findOverallBestAverage = useCallback(
		N => {
			if (times.length < N) return null;
			...
			for (let i = 0; i <= times.length - N; i++) {
				...
				const currentWindowAvg = calculateAverageFromValues(
					windowTimeValues,
					N
				);
	...
	const updateStats = useCallback(() => {
		...
		const averageTime =
			timeValues.reduce((sum, t) => sum + t, 0) / totalSolves;
		...
		for (const size of avgSizes) {
			...
			if (times.length >= size) {
				...
				if (overallBestForSize !== null) {
					if (
						newBestAverages[`ao${size}`] === null ||
						overallBestForSize < newBestAverages[`ao${size}`]
					) {
	...
	const startTimer = useCallback(() => {
		if (autoResetTimerIdRef.current) {
			// Should ideally be cleared by startPreparing
			clearTimeout(autoResetTimerIdRef.current);
			autoResetTimerIdRef.current = null;
		}
