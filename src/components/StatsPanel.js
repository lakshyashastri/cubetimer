...
const StatsPanel = ({
	...
	const {
		overview: isOverviewVisible,
		averages: isAveragesVisible,
		chart: isChartVisible,
		times: isTimesPanelVisible
	} = panelVisibility;
	...
	const getPanelClasses = (
		...
		if (panelVisibility[panelName]) {
			...
			if (panelName === "chart") {
				// Specific transform for chart when visible
		...
		<>
			...
			<div className={getPanelClasses("overview", styles.overviewPanel)}>
				...
				<div className={styles.panelContent}>
					{isOverviewVisible && (
						<Overview stats={stats} formatTime={formatTime} />
					)}
				</div>
			</div>
			...
			<div className={getPanelClasses("averages", styles.averagesPanel)}>
				<div className={styles.panelHeader}>
					<h3>Averages</h3>
					<button
						onClick={() => togglePanel("averages")}
						className={styles.closeBtn}
						title="Close Averages"
					>
						×
					</button>
				</div>
				<div className={styles.panelContent}>
					{isAveragesVisible && (
						<Averages
							averages={averagesData.averages}
							bestAverages={averagesData.bestAverages}
			...
			<div
				className={getPanelClasses(
					"times",
					styles.timesPanel,
					shouldTimesPanelShiftUp,
					styles.timesPanelShiftedUp
				)}
			>
				<div className={styles.panelHeader}>
					<h3>Times</h3>
					<button
						onClick={() => togglePanel("times")}
						className={styles.closeBtn}
						title="Close Times List"
					>
						×
					</button>
				</div>
