...
const PerformanceChart = ({ times, formatTime }) => {
	...
	const isPersonalBest = useCallback(
		(timeValue, solveIndexInAllTimesReversed, allTimesReversed) => {
			// `solveIndexInAllTimesReversed` is the index in `allTimesReversed` (0 = newest solve overall)
			const previousSolves = allTimesReversed
				.slice(solveIndexInAllTimesReversed + 1)
				.map(t => t.time);
			if (
				allTimesReversed.length > 0 &&
				solveIndexInAllTimesReversed === allTimesReversed.length - 1
			) {
				// This is the very first solve
				return true;
			}
	...
	const memoizedDrawGrid = useCallback(
		(
			ctx,
			padding,
			chartWidth,
	...
	const memoizedDrawTrendLine = useCallback(
		(
			ctx,
			pointsToDraw,
			padding,
			...
			const startYPos =
				padding.top +
				chartHeight -
				((startYVal - adjustedMin) / Math.max(1, adjustedRange)) *
					chartHeight;
			const endYPos =
				padding.top +
				chartHeight -
				((endYVal - adjustedMin) / Math.max(1, adjustedRange)) *
					chartHeight;
			const firstPointX = pointsToDraw[0].x;
			const lastPointX = pointsToDraw[n - 1].x;
			ctx.moveTo(firstPointX, startYPos);
			ctx.lineTo(lastPointX, endYPos);
			ctx.stroke();
			ctx.setLineDash([]);
			ctx.globalAlpha = 1;
		},
	...
	const memoizedDrawDataLine = useCallback((ctx, pointsToDraw) => {
		// Removed allTimesReversed from args, will get from closure/chartRenderData if needed by point structure
		...
		pointsToDraw.forEach(point => {
			const isActuallyTheOverallMostRecent =
				point.indexInAllTimesReversed === 0;
			...
			if (isPB && !isActuallyTheOverallMostRecent) {
				// Small inner dot for non-most-recent PBs
	...
	const memoizedDrawAxes = useCallback(
		(
			ctx,
			pointsToDraw,
			padding,
	...
	const executeDraw = useCallback(
		currentProgress => {
			...
			const pointsToRenderCount = Math.floor(
				allCalculatedPoints.length * currentProgress
			);
			const pointsToDraw = allCalculatedPoints.slice(
				0,
				pointsToRenderCount
			);
	...
	useEffect(() => {
		...
		const recentTimes = times.slice(-50); // Oldest of the (up to) 50 most recent solves is at index 0, newest at end.
		...
		const points = [];
		recentTimes.forEach((timeItem, indexInRecentTimes) => {
			// timeItem is oldest of the recentTimes at index 0
			const x =
				padding.left +
				(indexInRecentTimes / Math.max(1, recentTimes.length - 1)) *
					chartWidth;
			const y =
				padding.top +
				chartHeight -
				((timeItem.time - adjustedMin) / Math.max(1, adjustedRange)) *
					chartHeight;

			const indexInAllReversed = allTimesReversed.findIndex(
				t => t.id === timeItem.id
			);
	...
	useEffect(() => {
		if (
			times.length > prevTimesLengthRef.current &&
			prevTimesLengthRef.current !== 0
		) {
			setIsAnimating(true);
	...
	const handleMouseMove = e => {
		...
		const pointsToConsiderCount = isAnimating
			? Math.floor(allCalculatedPoints.length * animationProgress)
			: allCalculatedPoints.length;
		const visiblePoints = allCalculatedPoints.slice(
			0,
			pointsToConsiderCount
		);
		...
		if (closestPoint) {
			tooltip.innerHTML = `<div class="${styles.tooltipSolve}">Solve #${
				closestPoint.solve
			}</div><div class="${styles.tooltipTime}">${formatTime(
				closestPoint.time
			)}</div>`;
