...
const TimesList = ({ times, formatTime, onDeleteTime, onRequestClearAllTimes }) => {
  ...
  return (
    <>
      <div className={styles.timesControls}>
        <button onClick={onRequestClearAllTimes} className={styles.clearTimesBtn}>
          Clear All Times
        </button>
      </div>
          ...
          <div key={timeEntry.id} className={styles.timeEntry}>
            ...
            {pendingDeleteId === timeEntry.id ? (
              ...
              <button
                ...
                onClick={() => handleInitialDeleteClick(timeEntry.id)}
                title="Delete time"
              >
